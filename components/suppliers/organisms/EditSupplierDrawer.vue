<template>
  <right-drawer
    v-if="drawer"
    v-model="drawer"
    :title="$t('supplier.action.updateSupplier')"
    width="700px"
    @close="cancel"
  >
...
export default defineComponent({
  ...
  setup(_, { root, emit }) {
    ...
    const submit = () => {
      loading.value = true
      $api.supplier
        .updateSupplier(state.form.id, state.form)
        .then(() => {
          emit('success')
          $toast.success(root.$t('supplier.notice.updateSuccess'))
          drawer.value = false
        })
        .finally(() => {
          loading.value = false
